...
// ═══════════════════════════════════════════════════════════════
// NAVEGACIÓN SECUENCIAL ENTRE NOTAS
// ═══════════════════════════════════════════════════════════════

/**
 * Consulta la nota anterior, actual y siguiente para navegación secuencial.
 *
 * Prioriza notas PENDIENTES (PEND, PROC) en secuencia ascendente por ID/folio,
 * y continúa con notas CONFIRMADAS (CONF) y CANCELADAS en regresión descendente.
 * Aplica el filtro contextual de la bodega activa del usuario y scoping de roles.
 */
export async function fetchNavegacionNota(
  notaId: number,
  opciones?: {
    usuarioId?: number
    nivelAcceso?: number
    ...
    userBodegaIds?: number[]
  }
): Promise<NavegacionNota | null> {
  const supabase = await createClient()
  ...
  // 2. Filtro Contextual por Bodega Activa
  let bodegaFiltroNombre: string | null = null
  ...
  // 3. Ordenamiento inteligente: PEND/PROC primero (ASC), luego CONF/CANC (DESC)
  const sorted = [...allNotas].sort((a, b) => {
    ...
    if (!aIsPend && bIsPend) return 1
    if (aIsPend && bIsPend) return a.id - b.id // Secuencia ascendente para pendientes
    return b.id - a.id // Regresión descendente para confirmadas
  })
  ...
  const formatItem = (n: any): NavegacionNotaItem => {
