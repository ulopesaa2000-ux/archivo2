...
export function AddToQuoteButton({ producto, config }: AddToQuoteButtonProps) {
  ...
  // En modo catálogo (o cuando los precios están ocultos), no se muestra selector de piezas
  ...
  const handleAdd = () => {
    ...
    addItem({
      ...
      talla: '',
      color: '',
      cantidad: isCatalogoMode ? 1 : cantidad,
  ...
  const buttonText = isCatalogoMode
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-4 items-center mb-6">
        {/* Selector de cantidad (Solo visible en modo e-commerce con precios) */}
        {!isCatalogoMode && (
        ...
        <button
          onClick={handleAdd}
          disabled={isAdding}
          className="flex-1 bg-[#2D5A3D] text-white h-[52px] rounded-lg text-[13px] uppercase tracking-[0.05em] font-semibold hover:bg-[#1e3a2f] transition-all shadow-sm hover:shadow-md disabled:opacity-70 disabled:cursor-not-allowed text-center px-4"
        >
